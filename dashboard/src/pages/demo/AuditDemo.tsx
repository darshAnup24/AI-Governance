    ...
    <div className="space-y-5">
      ...
      <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-4">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-sm font-semibold text-slate-200">How Audit & Incidents works</span>
        </div>
        ...
        <p className="text-xs text-slate-500 mt-2">
          Every detection & policy decision is logged immutably. Use <strong className="text-slate-400">Generate Test Event</strong> below to inject sample data — it will appear in this table <em>and</em> on the{' '}
          <Link to="/governance/incidents" className="text-brand-400 underline hover:text-brand-300">Incidents board</Link>.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <div className="card border border-slate-800 text-center py-3">
          ...
          <p className="text-xs text-slate-500 mt-0.5">Events (last 20)</p>
        </div>
        <div className="card border border-slate-800 text-center py-3">
        ...
        <div className="space-y-4">
          <div className="card border border-slate-800">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-sm font-semibold text-slate-200">Generate Test Events</span>
            </div>
            <p className="text-xs text-slate-500 mb-3">Click any button to fire a test detection. Non-safe events auto-create an incident on the Incidents board.</p>
            <div className="space-y-2">
              {TEST_EVENTS.map(t => (
                <button key={t.label} onClick={() => generateEvent(t)}
                  disabled={generating !== null}
                  className="w-full flex items-center justify-between px-3 py-2.5 rounded-lg border border-slate-800 hover:border-slate-600 text-sm text-slate-400 hover:text-slate-200 transition-all disabled:opacity-50 group">
                  <span>{t.label}</span>
                  {generating === t.label
          ...
          {recentIncidents.length > 0 && (
            <div className="card border border-orange-500/20 bg-orange-500/5">
              <div className="flex items-center justify-between mb-2">
                ...
                <Link to="/governance/incidents" className="flex items-center gap-1 text-[10px] text-brand-400 hover:text-brand-300 transition-colors">
          ...
          <Link to="/governance/incidents"
            className="flex items-center justify-between p-3 rounded-xl border border-slate-800 hover:border-brand-500/40 hover:bg-brand-500/5 transition-all group">
            <div className="flex items-center gap-2">
              <AlertTriangle className="w-4 h-4 text-orange-400" />
              <div>
                <p className="text-sm font-medium text-slate-300">Incidents Board</p>
                <p className="text-xs text-slate-600">{(incidents as any[]).filter((i: any) => i.status !== 'RESOLVED').length} open incidents</p>
              </div>
            </div>
        ...
        <div className="lg:col-span-2 space-y-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-semibold text-slate-200">Live Audit Log</span>
              {isFetching && <Loader2 className="w-3 h-3 animate-spin text-slate-600" />}
            </div>
            <div className="flex items-center gap-2">
              {/* Filter */}
              ...
              <button onClick={() => refetch()} className="p-1.5 rounded hover:bg-slate-800 text-slate-600 hover:text-slate-300 transition-colors">
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-slate-800 bg-slate-900/50">
                  <th className="px-3 py-2.5 text-left text-slate-500 font-medium w-20">Time</th>
                  <th className="px-3 py-2.5 text-left text-slate-500 font-medium">Categories</th>
                  <th className="px-3 py-2.5 text-left text-slate-500 font-medium w-20">Risk</th>
                  <th className="px-3 py-2.5 text-left text-slate-500 font-medium w-20">Action</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-800/50">
                ...
                {(events as any[]).map((ev: any) => {
                  ...
                  return (
                    <tr key={ev.event_id} className="hover:bg-slate-800/20 transition-colors">
                      ...
                      <td className="px-3 py-2.5">
                        <div className="flex gap-1 flex-wrap">
                          {cats.length === 0
                            ...
                            : cats.map(c => (
                              <span key={c} className={`px-1.5 py-0.5 rounded text-[10px] font-medium ${CAT_COLORS[c] || 'bg-slate-800 text-slate-400'}`}>
                                {c.replace(/_/g, ' ')}
                              </span>
                            ))}
                        </div>
                      ...
                      <td className="px-3 py-2.5">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-slate-400 font-mono">{ev.risk_score || 0}</span>
                        </div>
                      </td>
                      <td className="px-3 py-2.5">
