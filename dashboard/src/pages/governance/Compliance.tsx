...
const statusColors: Record<string, string> = {
    ...
    IN_PROGRESS: 'text-blue-400', NOT_STARTED: 'text-[var(--muted-foreground)]', NON_COMPLIANT: 'text-red-400'
}
...
export default function Compliance() {
    ...
    return (
        <PageShell>
            <PageHeader
                badge="Regulatory Posture"
                title="Compliance"
                description="Track readiness across the active regulatory frameworks and run AI-assisted gap analysis from one shared governance shell."
                status={<StatusPill label="Live Scores" tone="live" pulse />}
                actions={
                        ...
                        return (
                            <button key={fw.id} onClick={() => setSelected(fw.id === selected ? null : fw.id)}
                                ...
                                <div className="flex items-center justify-between mb-3">
                                    <h3 className="text-sm font-semibold text-[var(--foreground)]">{fw.name}</h3>
                                    <StatusIcon className={`w-5 h-5 ${statusColors[check?.status || 'NOT_STARTED']}`} />
                                </div>
                                <div className="relative w-full h-2 bg-[var(--muted)] rounded-full overflow-hidden mb-2">
                                ...
                                <div className="flex justify-between text-xs">
                                    <span className="text-[var(--muted-foreground)]">{fw.questionCount} checks</span>
            ...
            {/* Checklist View */}
            {selectedFw && (
                <SurfaceSection title={`${selectedFw.name} Checklist`} description="Review the control questions contributing to this framework score.">
                    <div className="space-y-3">
                        {selectedFw.questions.map((q, i) => (
                            <div key={i} className="flex items-center gap-3 p-3 rounded-lg bg-[var(--muted)]/50">
                                <input type="checkbox" defaultChecked={i < Math.floor((selectedCheck?.score || 0) / 100 * selectedFw.questionCount)}
                                    className="w-4 h-4 rounded border-[var(--border)] bg-[var(--muted)] text-[var(--accent)]" />
            ...
            {/* Gap Analysis Result */}
            {gapResult && (
                <SurfaceSection title="AI Gap Analysis" className="border border-[var(--accent)]/20">
                    <pre className="text-sm text-[var(--foreground)] whitespace-pre-wrap leading-relaxed">{gapResult}</pre>
                </SurfaceSection>
            )}
        </PageShell>
    )
